...
import com.famsa.xmlmarshall.Campo;
import com.famsa.xmlmarshall.Campos;
import com.famsa.xmlmarshall.ConexionBD;
import com.famsa.xmlmarshall.Config;
import com.famsa.xmlmarshall.Folder;
import com.famsa.xmlmarshall.Hilo;
import com.famsa.xmlmarshall.Monitor;
import com.famsa.xmlmarshall.Tabla;
...
public class Cfg {
	...
	public Config getCFG() {
    	...
    	if(fileConf.exists() && !fileConf.isDirectory()) { 
    		...
    		try {
				configXML = (Config) jaxbUnmarshaller.unmarshal( new File(nomArcConf));
			} catch (JAXBException e) {
				logger.log(Level.SEVERE, e.toString(), e);
			}
    	}
    	...
    	creaCarpeta(configXML.getFolder().getParaProcesar());
	...
	private void creaDirectorioConfig() {
		
		File directoryXML = new File(tokens[0]+"\\Expedientes\\Config\\");
		if (! directoryXML.exists()) {
			directoryXML.mkdirs();
		}
	}

	private void creaArchivoXML() {

		nomArcConf = tokens[0]+"\\Expedientes\\Config\\config.xml";
		File fileXML = new File(nomArcConf);
		if(!fileXML.exists()) { 
		    
			Config config = new Config();
			config.setHilo(creaHilo());
			config.setConexion(creaConexion());
			config.setMonitor(creaMonitor());
			config.setFolder(creaFolder());
			config.setAutorizacion(creaAutorizacion());
			config.setPreautorizacion(creaPreAutorizacion());
	...
	private Folder creaFolder() {
		...
		folder.setParaProcesar(tokens[0]+"\\Expedientes\\Temporal\\XML\\");

...
public class BuscaConfiguracion implements IBuscaConfiguracion {

	final Logger logger = Logger.getLogger(ProcesaImagen.class.getName());
	...
	public Configuracion find() {
		...
		tokens = cadena.split("\\\\(?=[^\\\\]+$)");
		nomCarpeta = tokens[0]+"\\Expedientes\\Config\\";
		nomArcConf = nomCarpeta + "config.xml";
